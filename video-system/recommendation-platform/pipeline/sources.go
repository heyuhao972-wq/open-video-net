...
func LoadIndexVideos(q Query) []model.Video {
	...
	cfg := config.LoadConfig()
	if cfg.IndexBase == "" && len(cfg.ContentPlatforms) > 0 {
		return loadFromPlatforms(cfg.ContentPlatforms)
	}
	url := cfg.IndexBase + "/videos"

	res, err := http.Get(url)
	if err != nil {
		if len(cfg.ContentPlatforms) > 0 {
			return loadFromPlatforms(cfg.ContentPlatforms)
		}
...
func loadFromPlatforms(platforms map[string]string) []model.Video {
	out := make([]model.Video, 0)
	seen := map[string]bool{}
	for platformID, base := range platforms {
		if base == "" {
			continue
		}
		url := strings.TrimRight(base, "/") + "/videos"
		res, err := http.Get(url)
		if err != nil {
			continue
		}
